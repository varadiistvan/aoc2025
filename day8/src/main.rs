...
fn part_2(file: &str) -> usize {
    ...
    for i in 0..points.len() {
        for j in (i + 1)..points.len() {
            let dist = points[i].dist(&points[j]);
            let ordered_dist = ordered_float::NotNan::new(dist).unwrap();
            thing_vec.push(Reverse((ordered_dist, &points[i], &points[j])));
        }
    }
    ...
    while clusters.iter().map(|c| c.len()).sum::<usize>() < point_count || clusters.len() > 1 {
        thing.pop().unwrap();
    ...
    (last_thing.1.x * last_thing.2.x) as usize
}
